...
func NewTodooListPostgres(db *sqlx.DB) *TodoListPostgres {
...
func (r *TodoListPostgres) Create(userId int, list todo.TodoList) (int, error) {
	...
	createUserListQuery := fmt.Sprintf("INSERT INTO %s (user_id, lists_id) VALUES ($1, $2)", usersListsTable)
...
func (r *TodoListPostgres) Update(userId, listId int, input todo.UpdateListInput) error {
	...
	query := fmt.Sprintf("UPDATE %s tl SET %s FROM %s ul where tl.id = ul.lists_id AND ul.lists_id=$%d AND ul.user_id=$%d", todoListsTable, setQuery, usersListsTable, argId, argId+1)
